<x-layouts.app :title="'Admin: Manage Prompts'">
        ...
        <div class="bg-white/95 backdrop-blur-xl border border-white/20 shadow-[0_8px_30px_rgb(0,0,0,0.04)] w-full p-8 rounded-[32px]">
            <div class="flex justify-between items-center mb-8">
                ...
                <div class="flex gap-4">
                    <a href="{{ route('prompt.generator') }}" class="px-5 py-2.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 font-semibold rounded-xl text-[14px] transition-all">
            ...
            <div class="bg-[#FDFDFD] border border-[#E5E7EB] rounded-2xl p-6 shadow-sm mb-8">
                ...
                <form action="{{ route('admin.prompts.upload') }}" method="POST" enctype="multipart/form-data" class="flex gap-4 items-end">
                    @csrf
                    <div class="flex-1">
            ...
            <div>
                ...
                <div x-data="{ activeModule: null }" class="flex flex-col gap-4">
                    @forelse($promptsByModule as $moduleName => $prompts)
                        <div class="border border-gray-200 rounded-xl overflow-hidden bg-white">
                            <!-- Accordion Header -->
                            <button @click="activeModule = activeModule === '{{ $moduleName }}' ? null : '{{ $moduleName }}'" class="w-full px-6 py-4 flex justify-between items-center bg-gray-50 hover:bg-gray-100 transition-colors">
                                <div class="flex items-center gap-3">
                                    <svg class="w-5 h-5 text-gray-400 transition-transform duration-200" :class="{'rotate-180': activeModule === '{{ $moduleName }}'}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                    <h3 class="font-bold text-gray-900">{{ $moduleName }}</h3>
                                </div>
                                <span class="text-sm font-semibold text-gray-500 bg-gray-200 px-3 py-1 rounded-full">{{ $prompts->count() }}</span>
                            </button>
                            
                            <!-- Accordion Body -->
                            <div x-show="activeModule === '{{ $moduleName }}'" x-transition class="border-t border-gray-200 p-4 flex flex-col gap-3 bg-white">
                                @foreach($prompts as $prompt)
                                    <div class="border border-gray-100 rounded-lg p-4 flex items-center justify-between bg-gray-50">
                                        <div>
                                            <h4 class="font-bold text-gray-900">{{ $prompt->title }}</h4>
                                            <p class="text-xs text-gray-500 mt-1">Uploaded: {{ $prompt->created_at->format('M d, Y') }}</p>
                                        </div>
                                        <div class="flex gap-4 items-center">
                                            <button onclick="alert(`{{ addslashes(substr($prompt->content, 0, 500)) }}...`)" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">Preview</button>
                                            <form action="{{ route('admin.prompts.destroy', $prompt) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                                @csrf
                                                @method('DELETE')
